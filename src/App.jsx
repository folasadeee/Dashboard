...
function App() {
  ...
  function findHighestRatedMovie() {
    if (!list) {
      ...
      return (
        <>
        <img src={`https://image.tmdb.org/t/p/original/${posterPath}`} width="100px" />
        
        <h3 className="text-xl font-bold">{highestRatedMovie}</h3>
      <p className="text-lg"><b>Rating:</b> {(Math.round(highestRating) * 10) / 10} Stars ⭐</p>
  ...
  return (
    <>
      ...
      <h1 className="text-center mb-4 text-4xl font-extrabold leading-snug tracking-tight text-neutral-800 md:text-5xl lg:text-6xl ">Welcome to <span className="underline underline-offset-3 decoration-8 decoration-red-400 ">MovieHub</span>, your one-stop destination for all things film.</h1>
...
<hr />
        <p className="text-2xl"><h3 className="text-3xl font-bold py-2">Number of Voters</h3>{findNumberOfVoters()} 🙋</p>
</div>
          </div>
...
<div className="ratingSlider flex">
 ...
 <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded border border-red-500 active:ring-2 active:ring-red-300" onClick={handleLoadMore}>Load More</button>
          </div>
        ) : null}
      </main>
    </>
